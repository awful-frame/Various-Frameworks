...
if Spec("player") ~= 72 then return end

----------------------------- gui -----------------------------
local Builder = Tinkr.Util.GUIBuilder:New { config = "vfuryConfig" }
...
local general4 = Builder:Group {
    ...
    content = {
        ...
        Builder:Columns {                        Builder:Checkbox {
                key = "autoFear",
    ...
    content = {
        Builder:Rows {
            Builder:Columns {
                Builder:Slider {
                    key = 'erageSlider',
                    label = "Enraged Regeneration (Health % of player)",
        ...
        Builder:Columns {
            ...
            key = 'dSlider',
            label = "Swap to Defensive Stance (Health % of player)",
...
local TabGroup = Builder:TabGroup {
    ...
    tabs = {
        Builder:Tab {
            title = "General",
            content = { general1, DropdownGroup, DropdownGroup2 }
        }, Builder:Tab { title = "Offensive", content = { general4 } },
        Builder:Tab { title = "Defensives", content = { general2, SliderGroupDef } },
        Builder:Tab { title = "Drawings", content = { general3 } },
        Builder:Tab { title = "Keybinds", content = { KeybindInputGroup } }
    }
}
                    ...
                    content = {
                        Builder:Rows {
                            Builder:Text {
                                ...
                                color = "51322D"
                            }, Builder:Spacer {}, Builder:Spacer {},
                            Builder:Spacer {}, Builder:Spacer {}, Builder:Text {
                            text = "To be added.",
                            size = 12,
                            font = "Fonts\\FRIZQT__.TTF",
                            color = "51322D"
...
-- Create the erButton
local erButton = CreateFrame("Button", "erButton", UIParent,
...
local function UpdateButtonAppearance(button)
    if button.isEnabled then
        button:SetNormalTexture("Interface\\Icons\\ability_warrior_innerrage")
        button.text:SetText("Enabled")
    else
        button:SetNormalTexture("Interface\\Icons\\ability_warrior_rampage")
...
local pos = Builder:GetConfig("erButtonPos",
    { "BOTTOMLEFT", "ChatFrame1", "TOPLEFT", 30, 30 })
...
local function InitializeConfigDefaults()
    ...
    local defaultConfig = {
        ...
        erageSlider = 40, -- Default is 60% for Enraged Regeneration
        biSlider = 30,    -- Default is 50% for Bitter Immunity
        rcSlider = 25,    -- Default is 25% for Rallying Cry
        dSlider = 60,     -- Default is 45% for Defensive Stance swap

        -- keybinds
        burstBind = "SHIFT-ALT-0",
        sbeHealer = "SHIFT-ALT-1",
        sbTarget = "SHIFT-ALT-2",
        swTarget = "SHIFT-ALT-3",
        hlfTarget = "SHIFT-ALT-4",
        hleHealer = "SHIFT-ALT-5",
        hlfhealer = "SHIFT-ALT-6",
        chargeeHealer = "SHIFT-ALT-7",
        interveneHealer = "SHIFT-ALT-8",
        interveneLowest = "SHIFT-ALT-9",
        disarmFocus = "SHIFT-ALT-0",
        fearFocus = "SHIFT-1"
    }
...
Draw:Sync(function(draw)
    ...
    if Builder:GetConfig("enableMeleeDraw") == "yes" then
        local facing = Facing()
        if facing ~= nil and UnitExists("target") and
            UnitCanAttack("player", "target") and Alive("target") then
...
Elite:Populate({
    ...
    suddenDeathBuff = NewSpell(280776, { beneficial = true }),
    charge = NewSpell(100, {
        beneficial = true,
        targeted = true,
        damage = "physical",
    ...
    stormBoltDebuff = NewSpell(132169, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true
    }),
    ...
    spearOfBastion = NewSpell(376079, {
        damage = "physical",
        targeted = true,
        ignoreMoving = true,
        ignoreFacing = false
    }),
    sharpenBlade = NewSpell(198817, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true
    }),
    ...
    bladestorm = NewSpell(227847, {
        damage = "physical",
        targeted = true,
        ignoreMoving = true,
        ignoreFacing = true
    }),
    intervene = NewSpell(3411, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true,
        ignoreGCD = true
    ...
    bitterImmunity = NewSpell(383762, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true
    }),
    ...
    heroicLeap = NewSpell(6544, {
        targeted = false,
        ignoreMoving = true,
        ignoreFacing = true
    }),
    bersekerStance = NewSpell(386196, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true
    }),
    berserkerRage = NewSpell(18499, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true,
        ignoreGCD = true,
        ignoreDisoriented = true
    }),
    ...
    wreckingThrow = NewSpell(384110, {
        damage = "physical",
        targeted = true,
        ignoreMoving = true
    }),
    ignorePain = NewSpell(190456, { beneficial = true, ignoreFacing = true }),
    martialProwess = NewSpell(316440, { beneficial = true }),
    warBanner = NewSpell(236320, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true,
        targeted = false
    ...
    duel = NewSpell(236273,
        { damage = "physical", targeted = true, ignoreMoving = true }),
    ...
    enrageRegeneration = NewSpell(184364, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true
    }),
    ...
    -- buffs
    bloodcraze = NewSpell(393951, { beneficial = true }),
    markedForExecution = NewSpell(445584, { beneficial = true }),
    enrage = NewSpell(184362, { beneficial = true }),
    -- debuffs and talents
    ...
    -- Racials
    berserking = NewSpell(33697, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true,
        ignoreGCD = true
    }),
    bloodFury = NewSpell(20572, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true,
        ignoreGCD = true
    }),
    bloodFury2 = NewSpell(33702, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true,
        ignoreGCD = true
    }),
    bloodFury3 = NewSpell(20572, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true,
        ignoreGCD = true
    }),
    fireBlood = NewSpell(265221, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true
    }),
    escapeArtist = NewSpell(20589, {
        beneficial = true,
        ignoreMoving = true,
        ignoreFacing = true,
        ignoreGCD = true
...
function rslider(num)
    ...
    local random_offset = Random(-2, 2)

    -- Apply the offset to the input number
    local randomized_number = num + random_offset

    -- Ensure the number doesn't go below 1
    if randomized_number < 1 then return 1 end

    return randomized_number
end

Elite.StompTotems(function(totem, totemId)
    if totem1[totemId] and CanCast(e.ragingBlow.id) and
        HitboxDistance(totem, "player") < 6 then
        FaceSmooth(totem, e.ragingBlow.id, "Stomping - " .. Name(totem),
            GetDropdownValues())
        return
    end
    if totem2[totemId] and CanCast(e.heroicThrow.id) and
        HitboxDistance(totem, "player") < 30 and
        HitboxDistance(totem, "player") > 6 then
        FaceSmooth(totem, e.heroicThrow.id, "Stomping - " .. Name(totem),
            GetDropdownValues())
        return
    end
    if totem2[totemId] and CanCast(e.heroicThrow.id) and
    ...
    if totem2[totemId] and CanCast(e.slam.id) and
        HitboxDistance(totem, "player") < 6 then
        FaceSmooth(totem, e.slam.id, "Stomping - " .. Name(totem),
            GetDropdownValues())
        return
    end
end, 1.5)
...
Elite.Stomp(function(pobject, stompItemID)
    if pvpStomps[stompItemID] and CanCast(e.ragingBlow.id) and
        HitboxDistance(pobject, "player") < 6 then
        FaceSmooth(pobject, e.ragingBlow.id, "Stomping - " .. Name(pobject),
            GetDropdownValues())
        return
    end
    if pvpStomps[stompItemID] and CanCast(e.heroicThrow.id) and
        HitboxDistance(pobject, "player") < 30 and
        HitboxDistance(pobject, "player") > 8 then
        FaceSmooth(pobject, e.heroicThrow.id, "Stomping - " .. Name(pobject),
            GetDropdownValues())
        return
    end
    if pvpStomps[stompItemID] and CanCast(e.slam.id) and
        HitboxDistance(pobject, "player") < 6 then
        FaceSmooth(pobject, e.slam.id, "Stomping - " .. Name(pobject),
            GetDropdownValues())
        return
    end
end, 0.7)
local function defensiveBuffCheck(unit)
    if unit then
        for i = 1, #externalDefensiveBuff do
            ...
            if HasBuff(aura, unit) then return true end
        end
    end
    return false
end
...
local function TrinketsRacials()
    if not Used(e.recklessness.id, 3) or not Used(e.bladestorm.id, 3) then return end
...
impendingVictory:Callback(function(spell)
    ...
    if defensiveBuffCheck("player") and TrueHealthP("player") > 40 then
        return
    end
...
battleShout:Callback("maintainCombat", function(spell)
    if HasBuff(spell.id, "player") then return end
    if not Alive("player") then return end
    if UnitExists("target") and TrueHealthP("target") < 30 then return end
...
recklessness:Callback(function(spell)
    if Builder:GetConfig("autoReck") ~= "yes" then return end
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
    end
    if InPvp() and not IsPlayer("target") then return end
    if HitboxDistance("target") > 8 then return end
    if spell:Cast("target") then
        Alert(spell.id, "Reck", GetDropdownValues())
        return
    end
end)
...
odynsFury:Callback(function(spell)
    if Builder:GetConfig("autoOdyn") ~= "yes" then return end
...
bladestorm:Callback(function(spell)
    ...
    if not Used(e.rampage.id, 2) then return end
    if spell:Cast("target") then
        Alert(spell.id, "DMG", GetDropdownValues())
        return
    end
...
hamstring:Callback("slowPriority", function(spell)
    ...
    if IsMelee("target") and not DistanceMoved("target", 4) then return end
...
bitterImmunity:Callback(function(spell)
    if Builder:GetConfig("autoBI") ~= "yes" then return end
    if not HasTalent(383762) then return end
    if defensiveBuffCheck("player") then return end
    local bi = rslider(Builder:GetConfig("biSlider", 30))
    if TrueHealthP("player") > bi then return end
    if spell:Cast() then
        Alert(spell.id, "Defensive: Bitter Immunity! - LOW HP",
            GetDropdownValues())
        return
    end
end)
rallyingCry:Callback("lowest", function(spell)
    ...
    if defensiveBuffCheck("player") then return end
    local rc = rslider(Builder:GetConfig("rcSlider", 25))
...
enrageRegeneration:Callback(function(spell)
    if Builder:GetConfig("autoERage") ~= "yes" then return end
    if not Alive("player") then return end
    if TrueHealthP("player") > rslider(Builder:GetConfig("erageSlider", 40)) then
        return
    end
    if spell:Cast() then
...
defensiveStance:Callback(function(spell)
    ...
    if TrueHealthP("player") > rslider(Builder:GetConfig("dSlider", 60)) then
        return
    end
    if spell:Cast() then
        Alert(spell.id, "Swapped to Defensive Stance!", GetDropdownValues())
...
berserkerStance:Callback(function(spell)
    if Builder:GetConfig("autoStance") ~= "yes" then return end
    if not Alive("player") then return end
    if HasBuff(e.berserkerStance.id, "player") then return end
    if TrueHealthP("player") <= rslider(Builder:GetConfig("dSlider", 60)) then
        return
    end
    if spell:Cast() then
        Alert(spell.id, "Swapped to Berserker Stance!", GetDropdownValues())
        return
    end
end)
...
spellReflect:Callback("CC", function(spell)
    ...
    ObjectsLoop(enemies, function(enemy, guid)
        ...
        if IsTarget(enemy.key) then
            if Used(e.pummel.id, 1) then return true end
            if Used(e.shockwave.id, 1) then return true end
            if Used(e.stormBolt.id, 1) then return true end
...
charge:Callback("chargeeHealerBind", function(spell)
    ...
    if spell:Cast(eHealer.key) then return end
end)
...
berserkerRage:Callback("fHealer", function(spell)
    ...
    if fHealer and fHealer.key then
        ...
        if spell:Cast() then
            Alert(spell.id, "Removing cc on " .. Name(fHealer.key),
                GetDropdownValues())
            return
        end
    end
...
shatteringThrow:Callback(function(spell)
    ...
    ObjectsLoop(enemies, function(enemy, guid)
        ...
        if spell:Cast(enemy.key) then
            Alert(spell.id, "Shattering defensive on " .. Name(enemy.key),
                GetDropdownValues())
            return true
        end
    end)
...
onslaught:Callback(function(spell)
    ...
    if SpellCd(e.onslaught.id) > gcd() then return end
    if Rage() > 70 then return end
    if bcc("target") and not DebuffFrom("target", okayHit) then return end
    if spell:Cast("target") then
        -- Alert(spell.id, "Target", GetDropdownValues())
...
bloodthirst:Callback(function(spell)
    ...
    if bcc("target") and not DebuffFrom("target", okayHit) then return end
    if spell:Cast("target") then
        -- Alert(spell.id, "Maintain Buff", GetDropdownValues())
        return
    end
end)
...
heroicLeap:Callback("hlfHealer", function(spell)
    if not fHealer then return end
    if spell:Cast(fHealer.key) then return end
end)
...
charge:Callback("chargeCycloneCast", function(spell)
    if not InArena() then return end
    if SpellCd(e.pummel.id) ~= 0 then return end
    if SpellCd(e.charge.id) ~= 0 then return end
    ObjectsLoop(enemies, function(enemy)
        if not IsPlayer(enemy.key) then return false end
        if CastTarget(enemy.key, "player") and (SpellCd(e.spellReflect.id) == 0 and Used(e.spellReflect.id, 2)) then return false end
        if not IsPlayer(enemy.key) then return false end
        if not CanInterruptCast(enemy.key) then return false end
        if not Casting(33786, enemy.key) then return false end
        local distance = Distance(enemy.key, "player")
        if distance > 8 and distance <= 25 then
            if spell:Cast(enemy.key) then
                Alert(spell.id, "Charge " .. Name(enemy.key), GetDropdownValues())
                return true
            end
        end
...
intimidatingShout:Callback("bg", function(spell)
    ...
    for _, enemy in pairs(enemies) do
        if IsPlayer(enemy.key) and IsHealer(enemy.key) and not cc(enemy.key) and
            not IsTarget(enemy.key) then
...
ragingBlow:Callback("2stack", function(spell)
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
    end
...
ragingBlow:Callback("opportunist", function(spell)
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
    end
    if not HasTalent(444774) then return end
    if not HasBuff(456120, "player") then return end
    if bcc("target") and not DebuffFrom("target", okayHit) then return end
    if spell:Cast("target") then
        Alert(spell.id, "Target", GetDropdownValues())
        return
...
thunderClap:Callback("pve", function(spell)
    ...
    if HitboxDistance("target") > 8 then return end
    local enemyCount = AreaEnemies("player", 12)
...
avatar:Callback("pve", function(spell)
    ...
    if (UnitClassification("target") ~= "worldboss" and
            UnitClassification("target") ~= "rareelite" and
            UnitClassification("target") ~= "elite" and UnitClassification("target") ~=
            "rare") and TrueHealthP("player") > 30 and AreaEnemies("player", 10) < 2 and
        not IsPlayer("target") then
        return
    end
    if HitboxDistance("target") > 8 then return end
    if spell:Cast("target") then
...
execute:Callback("massacrePve", function(spell)
    ...
    if SpellCd(spell.id) > gcd() then return end
    if Health("target") > 35 and HasTalent(206315) then return end
    if spell:Cast("target") then return end
end, true)
...
shockwave:Callback("pve", function(spell)
    ...
    if Used(e.stormBolt.id, 1.5) then return end
    if Used(e.intimidatingShout.id, 1.5) then return end
    if AreaEnemies("target", 5) < 2 then return end
...
pummel:Callback("pve", function(spell)
    ...
    if CastingPercent("target") >= randomKick and CastingPercent("target") <= 94 and
        CanInterruptCast("target") then
        if spell:Cast("target") then
            Alert(spell.id, "Interrupting", GetDropdownValues())
            return
        end
    end
    if ChannelingPercent("target") >= randomKickChannel and
        CanInterruptChannel("target") then
        if spell:Cast("target") then
            Alert(spell.id, "Interrupting", GetDropdownValues())
            return
        end
...
spellReflect:Callback("pve", function(spell)
    ...
    if CastRemains("target") > 0 and CanInterruptCast("target") and
        SpellCd(e.pummel.id) < gcd() and InMelee("target") then
        return
    end
    if Used(e.pummel.id, 1) then return end
    if CastRemains("target") > 0 and CastRemains("target") <= Buffer() then
        if spell:Cast() then
            Alert(spell.id, "Reflecting", GetDropdownValues())
            return
        end
...
whirlwind:Callback("improvedBuffPve", function(spell)
    if HasTalent(436707) then return end
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
    end
    if HasBuff(85739, "player") then return end
    if HitboxDistance("target") > 8 then return end
...
rampage:Callback("enragePve", function(spell)
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
    end
    if GetBuffRemaining(184362, "player") > 1 then return end
    if spell:Cast("target") then
        Alert(spell.id, "Maintain Buff", GetDropdownValues())
        return
    end
end, true)
bloodthirst:Callback("enragePve", function(spell)
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
    end
    if GetBuffRemaining(184362, "player") > 1 then return end
...
recklessness:Callback("pve", function(spell)
    if Builder:GetConfig("autoReck") ~= "yes" then return end
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
    end
    if (UnitClassification("target") ~= "worldboss" and
            UnitClassification("target") ~= "rareelite" and
            UnitClassification("target") ~= "elite" and UnitClassification("target") ~=
            "rare") and Health("player") > 30 and AreaEnemies("player", 10) < 2 and
        not IsPlayer("target") then
        return
    end
    if HitboxDistance("target") > 8 then return end
...
odynsFury:Callback("pve", function(spell)
    if not HasTalent(spell.id) then return end
    if Builder:GetConfig("autoOdyn") ~= "yes" then return end
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
...
ragingBlow:Callback("pve", function(spell)
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
    end
    if spell:Cast("target") then
...
bloodthirst:Callback("pve", function(spell)
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
    end
    if spell:Cast("target") then
...
ragingBlow:Callback("procPve", function(spell)
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
    end
    if not HasBuff(396752, "player") then return end
    if spell:Cast("target") then
        Alert(spell.id, "Target", GetDropdownValues())
        return
    end
end, true)
bloodthirst:Callback("procPve", function(spell)
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
    end
    if not HasBuff(461288, "player") then return end
    if spell:Cast("target") then
        Alert(spell.id, "Target", GetDropdownValues())
        return
    end
end, true)
whirlwind:Callback("pve", function(spell)
    if not UnitExists("target") or not Alive("target") or not IsEnemy("target") then
        return
    end
    if HitboxDistance("target") > 8 then return end
...
berserkerStance:Callback("outCombatPve", function(spell)
    if Builder:GetConfig("autoStance") ~= "yes" then return end
    if not Alive("player") then return end
    if HasBuff(e.berserkerStance.id, "player") then return end
    if spell:Cast() then
...
local function inCombat()
    ...
    randomKick = Random(60, 80, 1) + Buffer()
    randomKickChannel = Random(25, 35, 2) + Buffer()

    if Builder:GetConfig("combatMode") == "yes" then
        if Builder:GetConfig("pveMode") == "no" then
            Alert(e.berserkerStance.id, "PvP Mode Active",
                GetDropdownValuesStatic())
        else
            Alert(e.defensiveStance.id, "PvE Mode Active",
                GetDropdownValuesStatic())
        end
    end
    ...
    if (healthStoneCount >= 1) and InPvp() and not pvpStoneUsed then
        if TrueHealthP("player") <= randomHealthstone and
            not defensiveBuffCheck("player") then
            pvpStoneUsed = true
            rmt("/use Healthstone")
        end
    elseif TrueHealthP("player") <= randomHealthstone and
        not defensiveBuffCheck("player") then
    ...
    ----------------- auto attack -----------
    if UnitExists("target") and HitboxDistance("target") < 6 and los("target") and
        IsEnemy("target") and Alive("target") and
        not Immune("target", e.ragingBlow.id, false) then
        AttackUnit("target")
    end
    if Builder:GetConfig("pveMode") == "no" then
        ----------------- stomp sect ------------
        ...
        shockwave("pve")
        --- Buffs
        whirlwind("improvedBuffPve")
        rampage("enragePve")
        bloodthirst("enragePve")
        --- CDs
        battleShout("maintainCombat")
        avatar("pve")
        recklessness("pve")
...
local function outCombat()
    ...
    if Prep() or not InPvp() then pvpStoneUsed = false end
    ------------ out combat logic --------------
    if not Mounted() then
        -- take healthstoen after 2 second delay
        -- GrabHealthstone(2,3)
        if Builder:GetConfig("pveMode") == "no" then
            defensiveStance("outCombat")
        else
            berserkerStance("outCombatPve")
        end
        battleShout("maintain")
        battleShout("maintainPvpArea")
        if Builder:GetConfig("pveMode") == "no" then
            if not HasDebuff(410126, "player") and
                not HasDebuff(410201, "player") then
                rampage()
                execute("massacre")
                execute()
                ragingBlow("opportunist")
