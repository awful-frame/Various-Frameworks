...
project.priest.discipline.rotation.dispel = function(type)
    ...
    local friend, debuff, friend_prio, has_touch_or_aff = project.util.dispel.magic.friend()
    local enemy, buff, enemy_prio = project.util.dispel.magic.enemy()
    ...
    if type == "pvp" then
        if awful.player.casting == "Mass Dispel"
                and not enemy
                and not friend
