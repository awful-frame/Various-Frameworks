...
infoTab:Text({
    text = awful.textureEscape(427453, 18, "0:2") .. " - Templar: |cFFF48CBA[Click] |cFFFFFFFF*recommended*",
    size = 11,
    header = false,
    paddingBottom = 10,
    OnClick = function(self, event)
        if CopyToClipboard("CYEA5ba6OK14IUITjS1kSUVJcBAAAYAAillZmtltZGLzM22MbAAAAAAY2aamFDzM2mxw2wsMLzwDwwMMDLLsBgxsMbzMzSzMTbzysNDAYDYAwYMjB") then
            awful.alert({
                message="Copied to clipboard, stop spamming me retard!",
                texture=86704,
                imgX = 1,
...
controlTab:Checkbox({
    text = awful.textureEscape(96231, 18, "0:12") .. " - Only Interrupt CC",
    var = "onlyInterruptCC",
    tooltip = "Automatically use Rebuke to interrupt crowd control spells only.",
    default = false,
    size = 11,
    paddingBottom = 30,
})
...
controlTab:Checkbox({
    text = awful.textureEscape(2139, 18, "0:12") .. " - Don't CC Interrupted Healers",
    var = "holdCCOnHealerLock",
    tooltip = "Automatically hold crowd control if the enemy healer is locked on their main school of magic.",
    default = true,
    size = 11,
    paddingBottom = 30,
})
...
defensiveTab:Checkbox({
    text = awful.textureEscape(633, 18, "0:12") .. " - Trust Priest Swap",
    var = "trustPriestSwap",
    tooltip = "Will hold Lay on Hands if priest has swap and isn't in CC.",
    default = false,
    size = 11,
})

defensiveTab:Checkbox({
    text = awful.textureEscape(59752, 18, "0:12") .. " - Auto Racial + Lay on Hands before Bubble",
    var = "autoTrinketLayOnHands",
    tooltip = "Automatically use Will to Survive/human racial and Lay on Hands before using Divine Shield.",
    default = true,
    size = 11,
})
...
defensiveTab:Slider({
    text = awful.textureEscape(642, 18, "0:0") .. " - Divine Shield Sensitivity",
    var = "bubbleSens",
    min = 0.6,
    max = 1,
    step = 0.01,
    default = 0.85,
    valueType = "x",
    tooltip = "Sensitivity multiplier for when to use Divine Shield."
})
