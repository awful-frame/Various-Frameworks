...
project.util.best_target.select = function()
    if not project.settings.misc_auto_target then
        return
    end
    ...
    if awful.target.guid == project.util.enemy.best_target.unit.guid then
        return
    end
    ...
    set_auto_target_cd()
    project.util.enemy.best_target.unit.setTarget()
end