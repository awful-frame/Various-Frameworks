...
local last_pve_msg = ""
project.util.debug.alert.pve = function(msg, txt)
    if not awful.DevMode then
        return true
    end

    if project.settings.debug_alerts_enabled and project.settings.debug_alerts["PVE"] then
        awful.alert(awful.colors.blue .. msg, txt)
        if project.settings.debug_alerts_print and last_pve_msg ~= msg then
            debug_print(awful.colors.blue .. msg)
            last_pve_msg = msg
        end
    end
